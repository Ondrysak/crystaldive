...
fn render_grav_wave(uv: vec2<f32>) -> vec3<f32> {
    ...
    let p = uv * (2.35 / max(u.zoom, 0.08));
    let r = length(p);
    let ang = atan2(p.y, p.x);

    let cycle = fract(t * 0.032);
    let chirp = cycle * cycle * (3.0 - 2.0 * cycle);
    let freq = mix(5.0, 24.0, chirp) * mix(0.75, 1.45, u.field_mix);
    let phase = t * mix(0.75, 3.5, chirp) + u.color_shift * TAU;
    let sep = mix(0.78, 0.18, chirp);
    let amp = (0.035 + 0.16 * u.iso_level) * (0.35 + 1.2 * chirp);
    ...
    let wave_ring = pow(wave, mix(5.0, 14.0, u.iso_level)) * smoothstep(2.8, 0.25, r);

    let arm_w = 0.035 + 0.025 * u.field_mix;
    ...
    let diff = hp * cos(2.0 * u.color_shift * TAU) + hx * sin(2.0 * u.color_shift * TAU);
    ...
    strain_col = grav_wave_hue_shift(strain_col, u.color_shift * 0.35);
