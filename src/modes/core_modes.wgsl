...
fn render_links(uv: vec2<f32>) -> vec3<f32> {
    ...
    let cp  = vec3<f32>(sin(az)*cos(el), sin(el), cos(az)*cos(el)) * (3.2 / mp(MP_ZOOM));
    let fwd = normalize(-cp);
    ...
    for (var i = 0; i < 90; i++) {
        ...
        if d < 0.003 {
            ...
            let hue  = fract(cf_val * 1.4 + cf2v * 0.5 + mp(MP_COLOR_SHIFT) + t * 0.04);
...
fn render_xrd(uv: vec2<f32>) -> vec3<f32> {
    ...
    let sc    = 0.75 / mp(MP_ZOOM);
    let sigma = max(0.014 - mp(MP_ISO_LEVEL) * 0.011, 0.003);
    let ewald_k = 20.0 + mp(MP_ISO_LEVEL) * 60.0;
    ...
    let hue_shift = mp(MP_COLOR_SHIFT) + t * 0.03;
    ...
    col += powder * ring_col * mix(0.5, 0.0, mp(MP_FIELD_MIX));
    col += spots  * spot_col * mix(0.4, 1.2, mp(MP_FIELD_MIX));

    let cf_bg = crystal_field(vec3<f32>(uv * 2.0 / mp(MP_ZOOM), t * 0.2)) * 0.015;
    col += cf_bg * u.crystal_color.xyz;

    let r = length(uv);
    col *= smoothstep(0.020, 0.036, r);
    col += exp(-r*r * 500.0) * vec3<f32>(1.0, 0.92, 0.70) * 0.25;
    col *= 1.0 - smoothstep(0.80, 0.98, r / (1.0 / mp(MP_ZOOM)));
...
fn render_recip3d(uv: vec2<f32>) -> vec3<f32> {
    ...
    let cp  = vec3<f32>(sin(az)*cos(el), sin(el), cos(az)*cos(el)) * (4.5 / mp(MP_ZOOM));
    let fwd = normalize(-cp);
    let rgt = normalize(cross(vec3<f32>(0.0, 1.0, 0.0), fwd));
    let up  = cross(fwd, rgt);
    let rd  = normalize(fwd + uv.x*rgt + uv.y*up);
    ...
    let G_scale  = 0.42;
    let base_r   = mix(0.025, 0.10, mp(MP_ISO_LEVEL));
    ...
    for (var i = 0; i < ng_r3d; i++) {
        let ga  = g_block.gamp[i];
        let amp = ga.w;
        let G   = ga.xyz * mp(MP_KSCALE) * G_scale;
        ...
        let hue  = fract(amp*1.8 + length(G)*0.9 + mp(MP_COLOR_SHIFT) + t*0.025);
        var surf = 0.5 + 0.5 * cos(TAU * (hue + vec3<f32>(0.0, 0.333, 0.667)));
