...
@fragment fn fs_field(f: VOut) -> @location(0) vec4<f32> {
    ...
    switch u.mode {
        ...
        case 20u { col = render_defect(uv); }
        case 21u { col = render_band_surface(uv); }
        case 22u { col = render_orbital(uv); }
        case 23u { col = render_spin_texture(uv); }
        case 24u { col = render_bz_path(uv); }
        case 25u { col = render_cdw(uv); }
        case 26u { col = render_quasicrystal(uv); }
        case 27u { col = render_thermal(uv); }
        case 28u { col = render_domain_wall(uv); }
        case 29u { col = render_fracture(uv); }
        case 30u { col = render_neutron(uv); }
        // ── newer batch (31..38) ──────────────────────────────────────────
        case 31u { col = render_berry(uv); }
        case 32u { col = render_hofstadter(uv); }
        case 33u { col = render_stm(uv); }
        case 34u { col = render_spectral(uv); }
        case 35u { col = render_vortex_knot(uv); }
        case 36u { col = render_bloch_wave(uv); }
        case 37u { col = render_plasmon(uv); }
        default  { col = render_nematic(uv); }
    }
    // vignette — skip for XRD, RECIP3D, KIKUCHI, HOFSTADTER, SPECTRAL (own boundaries)
    if u.mode != 10u && u.mode != 11u && u.mode != 19u && u.mode != 32u && u.mode != 34u {
        col *= 1.0 - 0.35*dot(f.uv*2.0-1.0, f.uv*2.0-1.0);
    }
    // tone-map — skip for XRD and HOFSTADTER (preserves sharp features)
    if u.mode != 10u && u.mode != 32u {
