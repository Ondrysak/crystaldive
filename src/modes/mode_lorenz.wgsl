...
fn render_lorenz(uv: vec2<f32>) -> vec3<f32> {
    ...
    let sigma = 10.0 + 1.5 * sin(t * 0.07);
    let rho   = 28.0 + 6.0 * sin(t * 0.05) + 8.0 * (u.field_mix - 0.5);
    let beta  = 8.0 / 3.0 + 0.4 * sin(t * 0.09);

    // Phase-space window centred on attractor centroid (0, 0, rho).
    // Display the (x, z) projection: uv.x -> x, uv.y -> z (flipped so up = +z).
    let win  = 30.0 / max(u.zoom, 0.08);
    ...
    let dt = mix(0.003, 0.010, u.iso_level);
    ...
    for (var step = 0i; step < 120i; step = step + 1i) {
        ...
        let hue     = fract(0.62 + 0.18 * s + 0.05 * p.z + u.color_shift + t * 0.03);
    ...
    let lobe_hue = fract(0.55 + 0.20 * prev_sign + u.color_shift);
