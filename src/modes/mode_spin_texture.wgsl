...
fn spin_texture_hue(h: f32) -> vec3<f32> {
    return 0.5 + 0.5 * cos(TAU * (h + u.color_shift + vec3<f32>(0.00, 0.33, 0.67)));
}
...
fn spin_texture_spin(k: vec2<f32>) -> vec3<f32> {
    ...
    let winding = 1.0 + floor(u.field_mix * 3.99);
    let a = atan2(k.y, k.x) * winding
          + 0.75 * f0
          + 0.20 * sin(r * (3.5 + 7.0 * u.iso_level) - t);
    let rashba = vec2<f32>(-sin(a), cos(a));

    let bloch_mix = 0.25 + 0.65 * u.w_motif / max(u.w_lattice + u.w_motif + u.w_band, 1e-3);
    let xy = normalize(mix(tangent, mix(rashba, radial, 0.30 + 0.35 * u.field_mix), bloch_mix)
                     + 0.12 * vec2<f32>(f1, -f0));

    let core_phase = r * (5.5 + 7.0 * u.iso_level) - t * 1.4 + f0 * 2.0;
...
fn spin_texture_dash(k: vec2<f32>, spin: vec3<f32>) -> f32 {
    let density = 11.0 + 13.0 * u.iso_level;
    let grid = k * density;
    let cell = floor(grid);
    let jitter = vec2<f32>(
...
fn render_spin_texture(uv: vec2<f32>) -> vec3<f32> {
    ...
    let r = length(k);
    let ring = 0.5 + 0.5 * sin(r * (13.0 + 12.0 * u.iso_level) - u.time * u.speed * 0.55);
